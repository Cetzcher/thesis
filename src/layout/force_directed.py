...
class ForceDirectedLayout(HeightMapLayout):
    ...
    def compute_attraction(self, node, other):
        ...
        ideal_distance = clamp(1/weight, 8, self.size)
    ...
    def apply_forces(self):
        for node in self.graph.G.nodes:
            ...
            mass = self.graph.Pr(node) * self.MASS_MULT
            force = self.force[node]
            velocity = mass * force
            new_pos = pos + velocity
            self.positions[node] = vec.Vector2(int(new_pos[0]), int(new_pos[1]))
